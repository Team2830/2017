...
public class Drivetrain extends Subsystem {
    ...
    public void driveTank(Joystick joystick){
		...
		//double rightCalc = leftStick.getRawAxis(3) * Math.abs(leftStick.getRawAxis(3)) + Math.copySign(.35,leftStick.getRawAxis(3));
		//double leftCalc = leftStick.getRawAxis(1) * Math.abs(leftStick.getRawAxis(1))+  Math.copySign(.35,leftStick.getRawAxis(1));;
		SmartDashboard.putNumber("leftCalc",leftCalc);
		SmartDashboard.putNumber("rightCalc",rightCalc);
		SmartDashboard.putNumber("left",joystick.getRawAxis(1));
		SmartDashboard.putNumber("right",joystick.getRawAxis(3));
		
		if (Math.abs(rightCalc) > .38 || Math.abs(leftCalc) > .38){
			robotDrive41.tankDrive(rightCalc, leftCalc);
		}
    }
}
